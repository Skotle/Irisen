(() => {
  ...
  function MBoardBanPanel({ bans = [], permissions = {}, feedback, onBan, onUnban }) {
    ...
    return h("section", { className: "m-panel m-stack" },
      h(MSectionHead, { eyebrow: "Moderation", title: "사용자 차단/해제" }),
      canBan ? h(React.Fragment, null,
        h("div", { className: "m-field" }, h("label", { htmlFor: "m-ban-uid" }, "회원 UID"), h("input", { id: "m-ban-uid", value: targetUid, onChange: (event) => setTargetUid(event.target.value), placeholder: "회원 차단 시 입력" })),
        h("div", { className: "m-field" }, h("label", { htmlFor: "m-ban-ip" }, "IP"), h("input", { id: "m-ban-ip", value: targetIp, onChange: (event) => setTargetIp(event.target.value), placeholder: "비회원/IP 차단 시 입력" })),
        h("div", { className: "m-field" }, h("label", { htmlFor: "m-ban-reason" }, "사유"), h("input", { id: "m-ban-reason", value: reason, onChange: (event) => setReason(event.target.value) })),
        h("div", { className: "m-field" }, h("label", { htmlFor: "m-ban-expires" }, "만료 시각"), h("input", { id: "m-ban-expires", type: "datetime-local", value: expiresAt, onChange: (event) => setExpiresAt(event.target.value) })),
        h("button", {
          type: "button",
      ...
      ) : h("div", { className: "m-feedback-error" }, "사용자 차단 권한이 없습니다."),
      bans.length ? bans.map((ban) =>
        h("article", { className: "m-panel m-stack m-staff-card", key: `ban-${ban.ban_id}` },
          h("strong", null, ban.target_uid ? `${ban.target_nick || ban.target_uid} (${ban.target_uid})` : `IP ${ban.target_ip || ""}`),
          h("div", { className: "m-muted" }, `사유: ${ban.reason || "없음"}`),
          h("div", { className: "m-muted" }, ban.expires_at ? `만료: ${formatDate(ban.expires_at)}` : "만료 없음"),
          canBan ? h("button", { type: "button", className: "m-btn m-btn-secondary", onClick: () => onUnban(ban.ban_id) }, "해제") : null
        )
      ) : h("div", { className: "m-empty" }, "현재 활성 차단 기록이 없습니다."),
      h(MFeedback, { feedback })
    );
  }

  function MBoardManageView({ session, gid, board, manageData, feedback, onBanBoardUser, onUnbanBoardUser, onSaveSubmanagerPermissions, onAppointSubmanager, onRevokeSubmanager, onTransferManager, onLogout, alarmCount }) {
    const permissions = manageData?.permissions || {};
        ...
        h("section", { className: "m-card m-stack" },
          ...
          canOpen
            ? h(React.Fragment, null,
              ...
              (permissions.canBanUser || permissions.isAdmin) ? h(MBoardBanPanel, { bans: boardBans, permissions, feedback, onBan: onBanBoardUser, onUnban: onUnbanBoardUser }) : null,
  ...
  function PostView({ session, gid, postNo, post, comments, feedback, voteFeedback, voteState, settings, manageData, onSubmitComment, onDeletePost, onDeleteComment, onVote, onScrapPost, onReportPost, onLikeComment, onReportComment, onBanBoardUser, onLogout, alarmCount }) {
    ...
    const canBanAuthor = !!manageData?.permissions?.canBanUser || !!manageData?.permissions?.isAdmin || ["1", 1, "admin", "operator"].includes(session?.memberDivision);
    const banAuthor = (item, sourceLabel) => {
      ...
      if (!targetUid && !targetIp) {
        setCommentFeedback({ type: "error", message: "차단할 UID 또는 IP를 찾을 수 없습니다." });
        return;
      }
      onBanBoardUser?.({ gallId: gid, targetUid, targetIp: targetUid ? "" : targetIp, reason: `${sourceLabel} 작성자 직접 차단`, expiresAt: "" });
    };
          ...
          post
            ? [
                ...
                h("div", { className: "m-inline", style: { marginTop: "14px" }, key: "actions" },
                  ...
                  canBanAuthor && (post.writer_uid || post.ip) ? h("button", { type: "button", className: "m-btn m-btn-secondary", onClick: () => banAuthor(post, "게시글") }, post.writer_uid ? "UID 차단" : "IP 차단") : null,
                    ...
                    ? comments.map((comment) =>
                        h("div", { className: ["m-post-row", "m-comment-row", `depth-${commentDepth(comment)}`, isDeletedComment(comment) ? "is-deleted" : "", isReplyComment(comment) ? "is-reply" : ""].filter(Boolean).join(" "), key: comment.id || comment.comment_id },
                          ...
                          isDeletedComment(comment) ? null : h("div", { className: "m-inline", style: { marginTop: "8px" } },
                            ...
                            canBanAuthor && (comment.writer_uid || comment.ip) ? h("button", { type: "button", className: "m-btn m-btn-secondary", onClick: () => banAuthor(comment, "댓글") }, comment.writer_uid ? "UID 차단" : "IP 차단") : null,
  ...
  function ProfileView({ session, profileData, feedback, onSaveProfile, onDeleteHistory, onFollowProfile, onUnfollowProfile, onBlockProfile, onUnblockProfile, onLogout, alarmCount }) {
    ...
    return h(React.Fragment, null,
      ...
      h("main", { className: "m-shell m-stack" },
        h("section", { className: "m-panel m-stack m-profile-card" },
          ...
          !profileData.ownerView && session?.loggedIn
            ? profileData.blockedByViewer
              ? h("button", { type: "button", className: "m-btn m-btn-secondary", onClick: () => onUnblockProfile(profileData.uid) }, "차단 해제")
              : h("button", { type: "button", className: "m-btn m-btn-secondary", onClick: () => onBlockProfile(profileData.uid) }, "사용자 차단")
            : null
        ),
        profileData.canEdit
          ? h("section", { className: "m-panel m-stack" },
              ...
              h("label", { className: "m-check-row" }, h("input", { type: "checkbox", checked: showPosts, onChange: (event) => setShowPosts(event.target.checked) }), h("span", null, "작성 글 공개")),
              h("label", { className: "m-check-row" }, h("input", { type: "checkbox", checked: showComments, onChange: (event) => setShowComments(event.target.checked) }), h("span", null, "작성 댓글 공개")),
        ...
        h("section", { className: "m-panel m-stack" },
          h(MSectionHead, { eyebrow: "Posts", title: "작성 글" }),
          profileData.postsHidden
            ? h("div", { className: "m-empty" }, "이 사용자는 작성 글을 비공개로 설정했습니다.")
            : profileData.posts?.length
              ? profileData.posts.map((post) => h(MLink, { href: `/m/board/${encodeURIComponent(post.gall_id)}/${post.post_no}`, className: "m-panel m-stack", key: `${post.gall_id}-${post.post_no}` },
                  h("div", { className: "m-post-title" }, post.title || "제목 없음"),
                  h("div", { className: "m-muted" }, `${post.gall_name || post.gall_id} · ${formatDate(post.writed_at)} · 조회 ${post.view_count ?? 0}`)
                ))
              : h("div", { className: "m-empty" }, "작성한 글이 없습니다.")
        ),
        h("section", { className: "m-panel m-stack" },
          h(MSectionHead, { eyebrow: "Comments", title: "작성 댓글" }),
          profileData.commentsHidden
            ? h("div", { className: "m-empty" }, "이 사용자는 작성 댓글을 비공개로 설정했습니다.")
            : profileData.comments?.length
              ? profileData.comments.map((comment) => h(MLink, { href: `/m/board/${encodeURIComponent(comment.gall_id)}/${comment.post_no}`, className: "m-panel m-stack", key: comment.id },
                  h("div", { className: "m-post-title" }, comment.post_title || "게시글"),
                  h("div", { className: "m-muted" }, comment.content || ""),
                  h("div", { className: "m-meta m-muted" }, `${comment.gall_name || comment.gall_id} · ${formatDate(comment.writed_at)}`)
                ))
              : h("div", { className: "m-empty" }, "작성한 댓글이 없습니다.")
        ),
        profileData.ownerView ? h("section", { className: "m-panel m-stack" },
          h(MSectionHead, { eyebrow: "Scraps", title: "스크랩한 글" }),
          profileData.scraps?.length
            ? profileData.scraps.map((post) => h(MLink, { href: `/m/board/${encodeURIComponent(post.gall_id)}/${post.post_no}`, className: "m-panel m-stack", key: `${post.gall_id}-${post.post_no}` },
                h("div", { className: "m-post-title" }, post.title || "제목 없음"),
                h("div", { className: "m-muted" }, `${post.gall_name || post.gall_id} · ${formatDate(post.scrapped_at || post.writed_at)}`)
              ))
            : h("div", { className: "m-empty" }, "스크랩한 글이 없습니다.")
        ) : null
      )
    );
  }
      ...
      h("main", { className: "m-shell m-stack" },
        h("section", { className: "m-panel m-stack" },
          ...
          !session?.loggedIn
            ...
            : h(React.Fragment, null,
                h("section", { className: "m-panel m-stack" },
                  h(MSectionHead, { eyebrow: "Preferences", title: "알림 설정" }),
                  notificationSettings
                    ? [
                        ["inAppEnabled", "앱 내 알림 받기"],
                        ["emailEnabled", "이메일 알림 받기"],
                        ["followPostEnabled", "팔로우 새 글 알림"],
                        ["commentEnabled", "새 댓글 알림"]
                      ].map(([key, label]) => h("label", { className: "m-check-row", key },
                    ...
                    : h("div", { className: "m-muted" }, "알림 설정을 불러오는 중입니다."),
                  h(MFeedback, { feedback: notificationFeedback }),
                  notificationSettings ? h("button", { type: "button", className: "m-btn m-btn-primary", onClick: saveNotificationSettings }, "설정 저장") : null
                ),
                alarms.length
              ? alarms.map((alarm) =>
  ...
  function App() {
    ...
    useEffect(() => {
      document.title = pageTitleForRoute({
        ...
        board: currentBoard,
        post: postData.post,
        profileUid: route.params.uid || session?.uid || "",
        searchQuery
      });
    }, [route, currentBoard, postData.post, session?.uid, searchQuery]);
    ...
    function banBoardUser(payload, reset) {
      if (!boardManageData?.permissions?.canBanUser && !boardManageData?.permissions?.isAdmin) {
        setBoardManageFeedback({ type: "error", message: "사용자 차단 권한이 없습니다." });
        return;
      }
      api("/api/features/moderation/board-ban", {
        ...
        if (!result?.success) {
          setBoardManageFeedback({ type: "error", message: result?.message || "사용자 차단에 실패했습니다." });
          return;
        }
        reset?.();
        setBoardManageData((current) => current ? { ...current, boardBans: result.data?.bans || current.boardBans || [] } : current);
        setBoardManageFeedback({ type: "success", message: "사용자를 차단했습니다." });
      }).catch((error) => setBoardManageFeedback({ type: "error", message: error.message || "사용자 차단에 실패했습니다." }));
    }

    function unbanBoardUser(banId) {
      if (!boardManageData?.permissions?.canBanUser && !boardManageData?.permissions?.isAdmin) {
        setBoardManageFeedback({ type: "error", message: "사용자 차단 해제 권한이 없습니다." });
        return;
      }
      api(`/api/features/moderation/board-ban/${encodeURIComponent(banId)}`, { method: "DELETE" })
        .then((result) => {
          if (!result?.success) {
            setBoardManageFeedback({ type: "error", message: result?.message || "차단 해제에 실패했습니다." });
            return;
          }
          ...
          setBoardManageFeedback({ type: "success", message: "차단을 해제했습니다." });
        })
        .catch((error) => setBoardManageFeedback({ type: "error", message: error.message || "차단 해제에 실패했습니다." }));
    }
    ...
    function blockProfile(targetUid) {
      ...
      if (!window.confirm(`${targetUid} 사용자를 차단할까요?`)) return;
      api(`/api/features/blocks/${encodeURIComponent(targetUid)}`, { method: "POST" })
        .then((result) => {
          ...
          const endpoint = route.params.uid ? `/api/profile/${encodeURIComponent(targetUid)}` : "/api/profile/me";
          return api(endpoint).then((profile) => setProfileData(profile?.success ? profile.data : null));
        })
        .then(() => setProfileFeedback({ type: "success", message: "사용자를 차단했습니다." }))
    ...
    function unblockProfile(targetUid) {
      api(`/api/features/blocks/${encodeURIComponent(targetUid)}`, { method: "DELETE" })
        .then((result) => {
          ...
          return api(`/api/profile/${encodeURIComponent(targetUid)}`).then((profile) => setProfileData(profile?.success ? profile.data : null));
        })
        .then(() => setProfileFeedback({ type: "success", message: "차단을 해제했습니다." }))
    ...
    if (route.name === "profile") return h(ProfileView, { session, profileData, feedback: profileFeedback, onSaveProfile: saveProfile, onDeleteHistory: deleteProfileHistory, onFollowProfile: followProfile, onUnfollowProfile: unfollowProfile, onBlockProfile: blockProfile, onUnblockProfile: unblockProfile, onLogout: handleLogout, alarmCount });
